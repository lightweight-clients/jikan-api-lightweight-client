...
describe.sequential('requests integration tests', () => {
  ...
  test('should call a method with no args', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(new URL('https://api.jikan.moe/v4/reviews/anime'));
  ...
  test('should call a method with a simple arg', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(new URL(`https://api.jikan.moe/v4/anime/${animeId}`));
  ...
  test('should call a method with query args', async () => {
    ...
    expect(response.data?.length).toBeGreaterThan(0);
    expect(
      response.data?.every(({ mal_id }) => typeof mal_id === 'number' && mal_id >= 1),
    ).toBeTruthy();
  }, TEST_TIMEOUT_MS);
});
